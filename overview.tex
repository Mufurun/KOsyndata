...
The SL is estimated by Y refering the research of sea level change over time by Tanabe, & Nakashima in 2026 (\url{https://www.sciencedirect.com/science/article/pii/S266603342600016X#bib34})
...
\begin{tikzpicture}[
    ...
    \node[dashed_node] (Y) at (2,5) {Y\textsubscript{T}};
    \node (T) at (6,2.5) {$\not$T};
    \node[dashed_node] (U) at (1,1) {U};
    \node[red, dashed_node] (SL) at (1.5,2.7) {SL\textsubscript{T}};
    \node[blue, rectangle, draw, minimum size=7mm, thick] (K) at (3,1) {K\textsubscript{T}};
    \node[red, solid_node] (S) at (3.5,3.2) {S\textsubscript{T}};
    \node (dK) at (4.5,1) {dK};
    \node (dS) at (4,2) {dS};
    
    \draw [->] (Y) -- (K);
    \draw [->, green!70!black, thick] (Y) -- (SL);
    \draw [->, green!70!black, thick] (Y) -- (S);
    \draw [->, red, thick] (SL) -- (K);
    \draw [->] (dS) -- (S);
    \draw [->] (dK) -- (K);
    \draw [->, red, thick] (S) -- (K);
    \draw [->] (U) -- (K);
\end{tikzpicture}
\caption{DAG of causalities with hierarchical modeling with stratified by T (Model 2) - See Figure 3 for notations. }
\end{figure}
...
\begin{tikzpicture}[
    ...
    \draw [->] (U) -- (K);
\end{tikzpicture}
\caption{DAG of causalities with some hierarchical modeling with stratified by T (Model 3) - causality of Y -> S are ignored as it is represented in auto-correlation between S and SL, though Y -> SL is needed to estimate hidden SL. See Figure 3 for notations. }
\end{figure}
...
\begin{itemize}
    ...
    \item Model 1: includes all the causation in Fig. 3 without any stratification: Model T by Y, Model SL by Y, Model S by Y and dS, Model K by S, SL, Y, dK, and U. 

    \item Model 2: stratifies data by T, and includes all the causation in Fig. 3: Model T by Y, Model SL by Y, Model S by Y and dS, Model K by S, SL, Y, dK, and U. 

    \item Model 3: stratifies data by T, and includes all the causation in Fig. 4: Model T by Y, Model SL by Y, Model S by dS, Model K by S, SL, Y, dK, and U with correlation between S and SL.
    \begin{itemize}
        \item Although Model 3 does not rely on Y to predict S, the causation is represented as correlation between S and SL. 
    \end{itemize} 
    \begin{itemize}
...
\item Model K
$$K \sim Normal(\mu_K, U) $$
$$logit(\mu_K) = \alpha _{K} + \beta _{K_S} \cdot S + \beta _{K_{SL}} \cdot SL + \beta _{K_{Y}} \cdot Y + \beta _{K_{dK}} * dK + \beta _{K_T}[T]$$
\begin{itemize}
    ...
    \item $\beta _{K_T}$ is regularized to maintain the efficiency. 

\end{itemize}

$$\alpha _K, \beta _{K_S }, \beta _{K_{SL} }, \beta _{K_Y }, \beta _{K_{dK}}  \sim Normal(0,0.1)$$
\begin{itemize}
    \item This normal distribution has to have the sigma of 0.1. Otherwise, parameter T does not move as expected.
    \item Priors below also has to be strange like this. 
...
$$\overline{\beta {K_T}} \sim Normal(0, 0.1)$$
$$\sigma_{\beta _{K_T}} \sim Exponential(1)$$
$$z_{\beta _{K_T}}[T] \sim Normal(0, 0.1)$$
...
$$logit(\mu_S) = \alpha _S + \beta _{S_Y} \cdot Y + \beta _ {S_{dS}} \cdot dS + \beta _{S_T}[T]$$
\begin{itemize}
    \item I am hoping/believing $\beta_{dS}$ is negative and $\beta_{Y_S}$ is positive 
\end{itemize}

$$\alpha_S, \beta _{S_Y}, \beta _{S_{dS}} \sim Normal(0,0.1)$$
$$\beta _{S_T} = \overline{\beta _{S_T}} + \sigma_{\beta _{S_T}} * z_{\beta_{S_T}}[T]$$
$$\overline{\beta _{S_T}} \sim Normal(0, 0.1)$$
$$\sigma_{\beta _{S_T}} \sim Exponential(1)$$
$$z_{\beta _{S_T}}[T] \sim Normal(0, 0.1)$$
...
\begin{itemize}
    ...
    \item Also, $SL$ has to be between 0 and 1. 
\end{itemize}
$$SL \sim Normal(inv_logit(6 * Y - 2) - 0.04, 0.05)$$
...
\begin{itemize}
    \item With a constraint that Y between 0 and 1. 
\end{itemize}
...
\item Model K
$$K \sim Normal(\mu_K, U) $$
$$logit(\mu_K) = \alpha _{K}[T] + \beta _{K_S}[T] \cdot S + \beta _{K_{SL}}[T] \cdot SL + \beta _{K_{Y}}[T] \cdot Y + \beta _{K_{dK}} * dK$$

\begin{itemize}
    \item Basically same as m1 but regularization (partial pooling). 
    \item This does not need correlation because multilevel models of S and SL includes the information of Y 
...
$$\sigma_{\alpha _K}, \sigma_{\beta _{K _S}}, \sigma_{\beta _{K _{Sl}}}, \sigma_{\beta _{K _Y}} \sim Exponential(6)$$
...
\end{itemize}
$$\beta _{K_{dK}} \sim Normal(0, 0.3)$$
$$U \sim Exponential(1)$$


\item Model S 
\begin{itemize}
    \item Belows are similar to M1, though:
    \item $\beta _{S_T}$ are removed. 
    \item partial pooling for the same reason for $\alpha_S$ and $\beta _{Y_S}$
\end{itemize}
$$S \sim Normal(\mu_S, \sigma_S)$$
$$logit(\mu_S) = \alpha _S [T] + \beta _ {S_{dS}} * dS + \beta _{S_Y} \cdot Y$$
...
$$z_{\alpha _S}[T] \sim Normal(0,0.1)$$
$$z _{\beta _{S_Y}}[T] ~ normal(0,0.1)$$
\begin{itemize}
    \item don't need regularization
\end{itemize}
$$\beta {S_{dS}} \sim Normal(0,0.3)$$
...
\section{The priors and statistical model for Model 3 (m3)}
...
$$logit(\mu_K) = \alpha _{K}[T] + \beta _{K_S}[T] \cdot S + \beta _{K_{SL}}[T] \cdot SL + \beta _{K_{Y}}[T] \cdot Y + \beta _{K_{dK}} * dK$$
\begin{itemize}
    \item Similar to the m2. 
    \item In this case (m3), we do include correlation between S and SL due to Y. 
\end{itemize}
$$\alpha _K = \overline{\alpha _K} + \sigma_{\alpha _K} * z_{\alpha _K}[T] $$
$$\beta _{K_Y } = \overline{\beta _{K _Y}} + \sigma_{\beta _{K _Y}} * z_{\beta _{K_Y}}[T] $$
$$\overline{\alpha _K}, \overline{\beta _{K _Y}} \sim Normal(0,0.3)$$
$$\sigma_{\alpha _K}, \sigma_{\beta _{K _Y}} \sim Exponential(1)$$
$$z_{\alpha _K}[T] \sim Normal(0,0.1)$$
$$z_{\beta _{K_Y}}[T] \sim Normal(0,0.1)$$
\begin{itemize}
    \item Correlaton between $\beta _{K_S}$ and $\beta _{K_SL}$
    \item This $compose_noncentered()$ is in rethinking package. 
    \item This is not too strong but good correlation
\end{itemize}
$$\beta _{K_S}, \beta _{K_SL} \sim NonCenteredMultiNormal(\sigma_{S, SL}, \rho _{S, SL}, z_{S, SL})$$
$$\sigma_{S, SL} \sim exponential(2)$$
$$ \rho _{S, SL} \sim lkj_corr_cholesky(1) $$
...
\item Model S 
\begin{itemize}
    \item This one does not include $\beta_{S_Y}$ since this does not multi-model the causality. 
    \item This is auto-correlation. 
\end{itemize}
...
\item Model SL by Y 
\begin{itemize}
    \item Belows are the same coding as m2
\end{itemize}
$$SL \sim Normal(inv_logit(6 * Y - 2) - 0.04, 0.05)$$
