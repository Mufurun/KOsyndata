...
m1.3<- ulam(
    ...
    # Main model K
        ...
        logit(mu_K) <- aK[T] + bK_S[T] * S + bK_SL[T] * SL[i]  + bK_dK * dK,
         ...
         vector [25]: bK_SL <- bK_SL_bar + sigma_bK_SL * z_bK_SL[T],
         c(aK_bar, bK_S_bar,  bK_SL_bar) ~ normal(0,0.3),
         c(sigma_aK, sigma_bK_S, sigma_bK_SL) ~ exponential(6), # want low variation among groups
         z_aK[T] ~ normal(0,0.1),
         z_bK_S[T] ~ normal(0,0.1),
         z_bK_SL[T] ~ normal(0,0.1),
         bK_dK ~ normal(0, 0.5),
...
- This sampling efficiency is still not great. Let me remove everything except the correlation. 
...
- This model is good enough with forgivable eff, though not perfect.
- I will have the posterior check just in case, but I want to note that this is not great. 
...
m3<- ulam(
    ...
    # Model S 
        ...
        bS_T_bar ~ normal(0, 0.1), # regularization
...
I have no idea why it does not model bK_T and bS_T vvery well... I will figure it out.
