    ...
    # Main model K
     ...
     # Model S 
        ...
        # don't need regularization
         ...
         vector[200]: SL ~ normal(inv_logit(6 * Y - 2) - 0.04, 0.1),
...
KbyS<- sapply(Sseq, # Tricky but t and t to calculate estimates by group, though dK does not have group so do not need 
    function (i) inv_logit(post$aK + post$bK_S * i + t(t(post$bK_SL) * colMeans(post$SL)) + post$bK_dK * d$dK )) 
...
KbySL<- sapply(SLseq, 
    function (i) inv_logit(post$aK + t(t(post$bK_S) * d$S) + post$bK_SL * i + post$bK_dK * d$dK )) 
...
KbydK<- sapply(dKseq, 
    function (i) inv_logit(post$aK + t(t(post$bK_S) * d$S) + t(t(post$bK_SL) * colMeans(post$SL)) + post$bK_dK * i )) 
...
SbydS<- sapply(dSseq, 
    function (i) inv_logit(post$aS + post$bS_dS * i )) # Used mean of each simulated data
...
m2<- ulam(
    ...
    # Main model K
        ...
        corr_matrix[2]:RhoSSL ~ dlkjcorr(2),# this is not too strong but good correlation
...
- Therefore, I will tackle this problem with the same approach 
