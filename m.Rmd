...
- I can remove bK_T or bS_T but this time, I want to make keep the information as other models keep it by stratification. 
...
m2<- ulam(
    ...
    # Main model K
        ...
        # This does not need correlation because multilevel models of S and SL includes the information of Y
...
- This is very high eff and no divergent, as I can see in my synthetic data (I do not know yours,,,). This is great. This model is reliably sampled to be used for analysis!
...
- Since I haven't touched since, I do not check the hidden parameters. 


```{r post2check, fig.show="hold", out.width="50%"}
...
### Compare Models 
```{r comp1, fig.show="hold", out.width="50%"}
# Compare models
...
=> Therefore, the 2model 2.3 is better than the other one with lower WAIC, and so on...
...
- Instead, this does not include multilevel model of Y -> S, because it's in correlation. 
...
Let me remove one more thing so that it corresponds m1.3. 
...
```{r post3cor, fig.show="hold", out.width="50%"}
...
```{r post3pred, fig.show="hold", out.width="50%"}
...
```{r comp3, fig.show="hold", out.width="50%"}
# Compare models
...
- Model 1 can be used without removing any parameters, since the model somtimes works fine. 
```{r forgetit}
rm(list=ls())
```